...
### レイアウト
- 16:9 横長 **ホワイトボードキャンバス** (`QuestionWall` / React Flow ベース)
- 各問いは `position_x` / `position_y` (0–100%) の座標で **付箋として配置**
- ヒーロー / 投稿ボタン / 「?」マーク / 日付 は **全て排除**
- ヘッダー: 左上に「Questions Display」、中央に「問い」、右上に「2026年5月」のような月スタンプ
- 右下隅: 小さな QR コード + 「問いを投稿する」キャプション
- 付箋: パステル系の単色背景、角丸 16px、軽い回転 (-2°〜+2°)、ソフトシャドウ
- `/board` ではパン/ズームを無効化(スクショ用に固定ビュー)
...
### 方針（確定）
- `/board` と **同じ `QuestionWall`** で問いを表示(座標も共有)
- `interactive=true` でパン/ズーム、付箋クリックで **拡大モーダル**、🤔 リアクション
- 上部に **Three.js 製の Big Q ヒーロー** (タップで一回転、黄色のドロップシャドウ付き)
- 上部に「問いを投稿する」ボタン(CTA)
- 目的: 掲示板で見た景色とスマホで開いた画面を同じに見せ、「掲示板 → スマホ → 投稿」を切れ目なく繋ぐ

### `/ask` の配置 UI
- 既存付箋を半透明で背景表示
- 黄色い新規付箋をドラッグして好きな場所に貼り付け
- 確定位置を `position_x` / `position_y` (0–100%) で `/ask/submit` に送信

### アーカイブ導線
- フッター付近に **「過去の問いを見る」リンク 1 本** を置く
- 押すと月別インデックス(`/archive`)へ → そこから `/archive/[YYYY-MM]` へ降りる(同じ `QuestionWall`)
...
2. **`/board` ルート実装** 🟡
   ホワイトボード(`QuestionWall`、React Flow)で `position_x/y` 付箋配置、`published=true && archived=false` フィルタ、ヘッダー月スタンプ + QR コード焼き込み。スクショ運用の心臓。

3. **個人ビュー (`/`) の実装** 🟡
   `/board` と同じ `QuestionWall`(pan/zoom + クリック拡大)を CTA + Three.js Q ヒーロー付きで表示。月リスト・ロール表示の扱いはここで決める。
